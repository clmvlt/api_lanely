...
@Schema(name = "UpdateGoodsTypeRequest",
        description = "Payload to update a goods type. Only non-null fields are applied (PATCH semantics). A changed name must remain "
                + "unique within the company (case-insensitive).")
public record UpdateGoodsTypeRequest(

        @Schema(description = "Display name / autocompletion label, unique within the company (case-insensitive)",
                example = "Pallet of auto parts", nullable = true)
        @Size(max = 120)
        String name,
        ...
        @Schema(description = "Whether goods of this type are dangerous by default (ADR)", example = "false", nullable = true)
